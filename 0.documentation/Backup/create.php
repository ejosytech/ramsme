...
$mobile_no = $name_value= $location = $sec_outst_dec21= $addinfo = $plot_no = $occupancy = $role = $email= $avenue = $street =  $effective_date = $sec_contr_dec21 = $infr_contr_dec21 = $password = $confirm_password = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    } elseif(!filter_var(trim($_POST["effective_date"]), FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z0-9\s]+$/"))))
    {
        ...
        // Validate Security conribution as at December, 2021
    if(empty(trim($_POST["sec_contr_dec21"]))){
        $sec_contr_dec21_err = "Please provide security conribution as at December, 2021";
    } elseif(!filter_var(trim($_POST["sec_contr_dec21"]), FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z0-9\s]+$/"))))
    {
       $sec_contr_dec21_err = "Please  provide Security conribution as at December, 2021";
    } else{
        $sec_contr_dec21 = trim($_POST["sec_contr_dec21"]);
    }
    
    if(empty(trim($_POST["sec_outst_dec21"]))){
        $sec_outst_dec21_err = "Please provide security oustanding as at December, 2021";
    } elseif(!filter_var(trim($_POST["sec_outst_dec21"]), FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z0-9\s]+$/"))))
    {
       $sec_outst_dec21_err = "Please  provide Security oustanding as at December, 2021";
    } else{
        $sec_outst_dec21 = trim($_POST["sec_outst_dec21"]);
    }
    
     // Validate Infrastructure conribution as at December, 2021
    if(empty(trim($_POST["infr_contr_dec21"]))){
        $infr_contr_dec21_err = "Please provide infrastructure conribution as at December, 2021";
    } elseif(!filter_var(trim($_POST["infr_contr_dec21"]), FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z0-9\s]+$/"))))
    {
       $infr_contr_dec21_err = "Please provide conribution as at December, 2021";
    } else{
        $infr_contr_dec21 = trim($_POST["infr_contr_dec21"]);
    }
    ...
    } else
    {
        ...
        $sql = "INSERT INTO users (mobile_no,name_value,plot_no, avenue, street, email, occupancy, role,location,effective_date,sec_contr_dec21,infr_contr_dec21,sec_outst_dec21,addinfo, password) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
         
        if($stmt = mysqli_prepare($link, $sql))
        {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssssssssss", $param_mobile_no, $param_name, $param_plot_no, $param_avenue, $param_street,$param_email,$param_occupancy,$param_role, $param_location,$param_effective_date,$param_sec_contr_dec21,$param_sec_outst_dec21, $param_infr_contr_dec21, $param_addinfo,$param_password);
