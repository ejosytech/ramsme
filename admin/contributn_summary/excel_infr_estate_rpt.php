...
//
            ...
            $sheet->setCellValue('A'.$row_header_main, 'INFRASTRUCTURE PAYMENT STATUS AS AT TODAY: '. $todaysdate);
          ...
          //COL RESIZING 
           ...
           // Attempt select query execution
                    $sql = "select DISTINCT users.name_value vname, users.infr_contr_dec21 contr,  users.mobile_no mobile, sum(pay_sec_update.amount) paid from users INNER JOIN pay_sec_update where users.mobile_no = pay_sec_update.mobile_no and pay_sec_update.service = 'infrastructure'  group by users.mobile_no";
                                       //
            //Obtain Total Count 
                            ...
                            $count = $row_header;
                                ...
                                { 
                                    $count++;
                                       ...
                                       $sheet->setCellValue('E'.strval($count),number_format($infr_fixed_amount,2));                                        
                                       $sheet->setCellValue('F'.strval($count),number_format(($row['contr']+$row['paid']),2)); 
                                       $sheet->setCellValue('G'.strval($count), number_format(($row['contr'] + $row['paid']) - $infr_fixed_amount,2));
           ...
           //
            ...
            $writer->save('excel_store/infr_update_report.xlsx');
            //
...
<html lang="en">
 ...
 ?>
    ...
    <div class="rf-register-form">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="mt-5 mb-3"> List of Members' Infrastructure Payment Status in Microsoft Excel Format</h2>
                    <p></p>
                    <p></p>
                    <a href="/admin/contributn_summary/excel_store/infr_update_report.xlsx" class="btn btn-danger ml-3">Download Report in Excel Format</a>
                    <p></p>
