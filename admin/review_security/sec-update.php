...
// Check existence of entryparameter before processing further
    if(isset($_GET["entry"]) && !empty(trim($_GET["entry"]))){
        ...
        $sql = "SELECT * FROM pay_sec_update WHERE service = 'security' and entry = ?";
...
if(isset($_POST["entry"]) && !empty($_POST["entry"]))
{
    ...
    if(empty($input_service)){
        $service_err = "Please enter the service paid for";     
    } else{
   ...
   $input_remark = trim($_POST["remark"]);
    ...
    // Check input errors before inserting in database
        ...
        $sql = "UPDATE pay_sec_update SET mobile_no=?, name_value=?, pay_date=?, amount=?, service=?, remark=? WHERE entry = ?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssi", $param_mobile_no, $param_name, $param_pay_date, $param_amount, $param_service, $param_remark, $param_entry);
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-xl-10">
            <div class="card border-0">
                ...
                <select name="service"  class="form-select" aria-label="Default select example" value="<?php echo $service; ?>"> placeholder="Specify what payment is meant for" required >
                    <option selected ="<?php echo $service; ?>"><?php echo $service; ?></option>
