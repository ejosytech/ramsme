...
$entry = $service = $mobile_no = $phone = $amount= $remark = $pay_date = $attachment = $name_value= "";
...
// Check existence of entry parameter before processing further
    ...
    //    {
       ...
       //$entry = intval($_GET['entry']);
        ...
        $sql = "SELECT * FROM payments WHERE service = 'infrastructure' and entry = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    header("location: infr_error.php");
    ...
    $sql = "INSERT INTO pay_sec_update (mobile_no, pay_date, name_value, amount, service, attachment,remark) VALUES (?,?,?,?,?,?,?)";
        ...
        {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssss", $param_mobile_no, $param_date,$param_name, $param_amount, $param_service, $param_attachment,$param_remark );
           ...
           $param_remark = $remark;
            ...
            if(mysqli_stmt_execute($stmt)){
                // Redirect to login page
                   header("location: infr_pay_update_details_edit.php");
                  //include('infr_pay_update.php');
            } 
            else
            {
