...
$query = "SELECT * FROM pay_sec_update where service = 'infrastructure' " . $condition . "LIMIT " . $start_from . "," . $record_per_page;
...
while ($row = mysqli_fetch_array($result)) {
    $output .= '  
            <tr readonly>
                                      ...
                                      <td>' . $row['service'] .'</td>
                                      <td>' . $row['remark'] . '</td>
                                      <td>' . '<a href="infr-update.php?entry='. $row['entry'] .'" class="mr-3" title="Update Record" data-toggle="tooltip"><span class="fa fa-pencil"></span></a>'. '</td>
                                      <td>' . '<a href="infr-delete.php?entry='. $row['entry'] .'" title="Delete Record" data-toggle="tooltip"><span class="fa fa-trash"></span></a>'. '</td>
...
$page_query = "SELECT * FROM pay_sec_update where service = 'infrastructure'" . $condition;
$page_result = mysqli_query($link, $page_query);
$total_records = mysqli_num_rows($page_result);
$total_pages = ceil($total_records / $record_per_page);
for ($i = 1; $i <= $total_pages; $i++) {
    $output .= "<span class='edit_pagination_link' style='cursor:pointer; padding:6px; border:1px solid #ccc;' id='" . $i . "'>" . $i . "</span>";
}
$output .= '</div><br /><br />';
echo $output;
?>  
