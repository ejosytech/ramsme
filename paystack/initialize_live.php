...
{     
    ...
    // Compute Personal Charge 
    if ($amount>200000)
    {$charge = 10000; }  // Charge N100 for values more than N2000
    else 
    {$charge = 5000;}    // Charge N50 for values less than N2000
...
$posted_data = array(
         'amount'=>$amount,
          'email'=>$email,
          'subaccount'=> 'ACCT_ol41meops2yc19n',
         // 'transaction_charge' => $charge
         // 'bearer' => 'subaccount' Commenting this out implies that Charge is to be beared by main account
        );
...
curl_setopt_array($curl, [CURLOPT_URL => "https://api.paystack.co/transaction/initialize",
  ...
  CURLOPT_HTTPHEADER => [
    "authorization: Bearer sk_live_010265d1f0bd6dfe65fe05f7004b97fcfa9b6df1", //replace this with your own test key
